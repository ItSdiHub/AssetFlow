...
class OrganizationalManager {
  ...
  async onEmployeeOfficeChange() {
    ...
    if (!deptId) {
      officeSelect.innerHTML = `<option value="">-- ${AppState.lang === "ar" ? "اختر قسم أولاً" : "Select department first"} --</option>`;
    ...
    if (!dept || !dept.locationId) {
      officeSelect.innerHTML = `<option value="">-- ${AppState.lang === "ar" ? "لا يوجد موقع مرتبط بهذا القسم" : "No location linked to this department"} --</option>`;
      officeSelect.disabled = true;
      return;
    }
    ...
    officeSelect.innerHTML = `<option value="">-- ${AppState.lang === "ar" ? "لم يتم تحديد مكتب (اختياري)" : "No office selected (optional)"} --</option>` +
      officeOptions
